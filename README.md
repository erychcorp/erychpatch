...
* Game:Print() Hooks - may come in handy for developing/breaking down LUA scripts. PCF had used it practially everywhere for debugging their every little thing, but threw it out of all release builds at some point (except PainEd ofc, it's engine.dll has always had built-in reader for EDITOR.OutputText without need for con object). Brought it back in attempt to better analyze entities.
